...
export default function PostAdPage() {
  ...
  // Note: Assuming 'availableBalance' is passed in the user object or logic
  // If not explicitly in user object, we use currentBalance for logic
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. SERVER-SIDE BALANCE CHECK (Crucial fix)
      ...
      if (!checkRes.ok) {
        ...
        return // Stop execution before image upload
      }
        ...
        <div className="mx-auto max-w-2xl">
          <Card>
            ...
            <CardContent>
              ...
              {/* Balance Section */}
              <div className="mb-6 rounded-lg bg-muted p-4 space-y-1">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-muted-foreground">Total Balance:</span>
                  <span className="font-medium text-foreground">Rs {currentBalance.toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-muted-foreground">Available Balance:</span>
                  <span className={cn("font-bold", hasInsufficientBalance ? "text-destructive" : "text-green-600")}>
                    Rs {(currentBalance).toLocaleString()} - Rs {price.toLocaleString()} = Rs {(currentBalance - price).toLocaleString()}
                  </span>
                </div>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {/* Ad Type with Custom Hover Colors */}
                ...
                <div className="space-y-2">
                  ...
                  {imagePreview ? (
                    ...
                    <div
                      ...
                      className={cn(
                        "flex h-40 w-40 cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-border transition-colors hover:border-primary hover:bg-muted",
                        isCompressing && "opacity-50 cursor-not-allowed"
                      )}
                    >
                      {isCompressing ? <Loader2 className="h-8 w-8 animate-spin text-muted-foreground" /> : <Upload className="h-8 w-8 text-muted-foreground" />}
                  ...
                  <p className="text-xs text-muted-foreground">Max 12 MB. JPG, PNG, or GIF.</p>
                </div>
